...
const (
	...
	copilotHooksDir     = ".github"
	copilotHooksFile    = "hooks.json"
...
// writeCopilotHookFile reads an existing .github/hooks.json (if present), removes
// any lefthook-managed entries (commands produced by lefthook install), merges
// in the new entries derived from events, and writes the result back.
//
// Copilot hooks use a flat structure:
//
//	{
//	  "version": 1,
//	  "hooks": {
...
func (l *Lefthook) writeCopilotHookFile(path string, events map[string]string, bin string, quoteBin bool) error {
	return l.writeFlatHookFile(path, events, copilotHooksVersion, stripCopilotLefthookEntries, bin, quoteBin)
}
...
// stripCopilotLefthookEntries reads the "hooks" object from an existing Copilot
// hooks map and drops any entries whose command was generated by lefthook.
// Non-lefthook entries are preserved as-is.
func stripCopilotLefthookEntries(existing map[string]any) map[string]any {
	return stripFlatLefthookEntries(existing)
}
